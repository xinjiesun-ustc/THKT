...
data = pd.read_csv(
    '../data/EdNet/ednet_kt1_4LPKT.csv',
    usecols=[ 'start_time', 'user_id', 'skill', 'problem_id', 'correct']
).dropna(subset=['skill', 'problem_id']).sort_values(by=['start_time', 'problem_id'])
...
def parse_student_seq(student):
    ...
    p = [problem[p] for p in seq.problem_id.tolist()]
...
sequences2tl(split_and_pad_records(train_data,120), '../data/EdNet/train_ednet_kt1_4LPKT.txt')
sequences2tl(split_and_pad_records(test_data,120), '../data/EdNet/test_ednet_kt1_4LPKT.txt')
